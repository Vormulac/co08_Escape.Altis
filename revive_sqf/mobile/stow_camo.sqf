// stow_camo.sqf
// norrin, MARCH 2013
private ["_vcl","_pos_vcl","_dir_vcl","_ammo_crates","_shed"]; 
_vcl 		 = _this select 0;
_pos_vcl 	 = getPos _vcl;
_dir_vcl 	 = getDir _vcl;
_ammo_crates = nearestObjects [_vcl, ["ReammoBox","ReammoBox_F","EAST_Box_Base","IND_Box_Base","NATO_Box_Base","Box_Ammo_F"], 10];
{if (!isNull _x) then {deleteVehicle _x}} forEach _ammo_crates;
_shed = nearestObject [_vcl, "Land_FirePlace_F"];
deleteVehicle _shed;
//reset enemy spawn variables
no_base_1 = false;
no_base_1b = false;
publicVariable "no_base_1";
publicVariable "no_base_1b";
_vcl setFuel 1;
NORRN_camo_net = false;
publicVariable "NORRN_camo_net";
NORRN_camo_reset = true;
publicVariable "NORRN_camo_reset";
_vcl removeAction NORRN_l_remove_spawn_act;