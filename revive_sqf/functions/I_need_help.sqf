// I_need_help
// MAY 2013 - norrin
private ["_can_revive","_can_revive_2","_goto_revive_distance","_AI_smoke","_AI_cover","_call_for_AI_help","_smoke_shell_types","_helpComments","_revive_units","_potential_revivers","_possible_reviver","_reviver","_mags","_smoke_rounds","_newPos"];
_unit 						= player;
		...
		if (!isplayer _reviver) then {
			...
			if (currentCommand _reviver  == "stop") then	{	
				if (!local _reviver) then {
					norrnRpvEH = [NORRN_rjoinGroup, [_reviver,grpNull]];
			...
			if (local _reviver) then {
				_reviver doMove (getPos _unit);
			} else {
				norrnRpvEH = [NORRN_rdoMove, [_reviver,(getPos _unit)]];
				publicVariable "norrnRpvEH";
			};	
			...
			norrnRpvEH = [NORRN_rhint, [_reviver,_unit,0]];
			publicVariable "norrnRpvEH";
			hint "";
		};
	};
		...
		if (!isplayer _goto_protector) then	{		
			...
			if (currentCommand _goto_protector == "stop") then {	
				if (!local _goto_protector) then {
					...
					norrnRpvEH = [NORRN_rjoinGroup, [_goto_protector,grpNull]];
					publicVariable "norrnRpvEH";
				}else{ 
			...
			_newPos = [((getPos _unit) select 0) + 5, ((getPos _unit) select 1) + 5, ((getPos _unit) select 2)];
			if (local _goto_protector) then {
				_goto_protector doMove _newPos;
			} else {
				norrnRpvEH = [NORRN_rdoMove, [_reviver,_newPos]];
				publicVariable "norrnRpvEH";
			};
		};
	};		
