...
while {!_destroy} do {
	...
	if (_follow_cam == 1) then {
		if (!dialog) then {if (!isNull unconscious_body) then {_target = unconscious_body; NORRN_REVIVE_cam camsettarget _target;NORRN_REVIVE_cam cameraeffect ["internal", "back"];NORRN_REVIVE_cam camsetrelpos [-3,1,1];NORRN_REVIVE_cam camcommit 1};};
