// begin heal.sqf
// MAY 2013 - norrin 
private ["_array","_name","_wounded","_QG_animation","_medpacks","_reward_function"];
...
if (!isplayer _name) then
{
	...
	if (_QG_animation == 1) then 
	{
		...
		sleep 0.02;
		if (local _wounded) then {_wounded setDir 170} else {
			norrnRpvEH = [NORRN_rsetDir, [_wounded,170]];
