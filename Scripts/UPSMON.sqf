// =========================================================================================================
//  UPSMON - ARMA 3
//  Currently Maintained by the ARMA community
//  
//  Thanks Go Out To Monsada, Kronzky, Rafalsky for creating the ultimate patrol script for ARMA. 
//  We hope to continue all that awesome work.
//
// =========================================================================================================
...
// == set "UPSMON_grpid" to units in the group and EH===============================  
	{
		...
		{//civ
			if (! isnil "_x") then {
				...
				_x removeAllEventHandlers "killed"; //ToDO verify impact of this...
				sleep 0.05;
...
_driver = objnull;
_fortify = false;
_buildingdist= 75;//Distance to search buildings near
_Behaviour = "SAFE"; 
_grp = grpnull;
_grp = group _npc;
_template = 0;
...
// TBD: add to global side arrays?
	...
	// create the clones
		for "_grpcnt" from 1 to _copies do {
			...
			[_lead, drn_var_Escape_enemyMinSkill] call EGG_EVO_skill;
			...
			_i=0;
			{
				...
				if (_i>1) then {
					...
					[_newunit, drn_var_Escape_enemyMinSkill] call EGG_EVO_skill;
	...
	if !_nosmoke then {
		...
		if ( !isNull (_target) && alive _target ) then {
			...
			if (_fightmode != "fight" ) then {
				_fightmode = "fight";
				_npc setCombatMode "YELLOW";  // !R  // ToDo check impact of "RED"
		...
		if (_ambush && !_ambushed) then {
			...
			{			
				...
				_roads = _mineposition nearroads KRON_UPS_ambushdist; // org value 40 - ToDo check KRON_UPS_ambushdist value
		if (KRON_UPS_Debug>0) then {player sidechat format["%1: Roads #:%2",_grpidx, (count _roads)]}; 
		if (KRON_UPS_Debug>0) then {diag_log format["%1: Roads #:%2",_grpidx, (count _roads)]}; 
			...
			{		
				...
				sleep 0.5;
				_x stop true;	
				_x setUnitPos "DOWN";	// ToDo check double set DOWN?				
			}foreach units _npc;		
				...
				if ( _dist <= _closeenough/2 ) then {	
					...
					if ((_nomove == "NOMOVE" && _rnd < 25) && !_reinforcementsent) then {		
						...
						if (_rnd < 80) then {
							...
							_Behaviour =  "AWARE"; // ToDo check impact "STEALTH"; 
						};	
						_wptype = "MOVE";
						_npc setCombatMode "RED";							
					}
					...
					_speedmode = "NORMAL";  
					_wpformation = "VEE"; //or WEDGE	// ToDo check if WEDGE should be prefered
					_unitpos = "AUTO";	
...
//	if (KRON_UPS_Debug>0) then {player sidechat format["%1 rea=%2 wai=%3 tim=%4 tg=%5 %6",_grpidx,_react,_waiting,_timeontarget,typeof _target,alive _target]};							
	...
	{		
		...
		if (alive _npc) then 
		{
			...
			if (!_gothit && { _isSoldier } && { !_GetIn_NearestVehicles } && { _fightmode != "walk" } ) then 
			{					
				_dist2 = _dist / 4;
				if ( _dist2 <= 100 ) then {
					_unitsIn = [];					
